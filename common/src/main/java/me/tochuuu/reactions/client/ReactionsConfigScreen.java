...
public final class ReactionsConfigScreen extends Screen {
    ...
    @Override
    protected void init() {
        compactLayout = this.width < BASE_FACE_SIZE + PANEL_WIDTH + 56 || this.height < 300;
        if (compactLayout) {
            initCompact();
            return;
        }

        int availableFaceWidth = this.width - PANEL_WIDTH - 56;
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(this.font, Component.literal("Eyes " + config.eyeWidth + "x" + config.eyeHeight), faceX, labelY, 0xFFA0A0A0);
        ...
        if (sizeLimitMessageTicks > 0) {
            graphics.text(this.font, Component.literal("Eye size limit reached"), panelX, Math.min(this.height - 38, eyeHeightRowY + BUTTON_HEIGHT + 6), 0xFFFF6060);
        }
    }
