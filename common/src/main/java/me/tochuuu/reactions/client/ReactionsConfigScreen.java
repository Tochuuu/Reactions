...
public final class ReactionsConfigScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(this.font, Component.literal("Skin UV picker"), faceX, faceY - 12, 0xFFA0A0A0);
        ...
        graphics.text(this.font, Component.literal("Editing: " + mode.label), faceX, labelY, mode.color);
        if (!compactLayout) {
            graphics.text(this.font, Component.literal("Eyes " + config.eyeWidth + "x" + config.eyeHeight), faceX, labelY + 11, 0xFFA0A0A0);
        }

        if (!compactLayout) {
            graphics.text(this.font, Component.literal("Animation"), panelX, panelY - 11, 0xFFA0A0A0);
        }
        graphics.text(this.font, Component.literal("UV parts"), panelX, partHeaderY, 0xFFA0A0A0);
        graphics.text(this.font, Component.literal("Eye size"), panelX, sizeHeaderY, 0xFFA0A0A0);
        graphics.text(this.font, Component.literal("Width: " + config.eyeWidth), panelX, eyeWidthRowY + 6, 0xFFFFFFFF);
        graphics.text(this.font, Component.literal("Height: " + config.eyeHeight), panelX, eyeHeightRowY + 6, 0xFFFFFFFF);
        ...
        if (!compactLayout && detailsY > 0 && detailsY < this.height - 46) {
            ...
            graphics.text(this.font, Component.literal("Eyelid: " + uv(config.eyelidColorX, config.eyelidColorY)), panelX, detailsY + 33, 0xFFFFC94A);
        }

        if (sizeLimitMessageTicks > 0) {
            int messageY = compactLayout ? Math.max(20, panelY - 12) : Math.min(this.height - 38, detailsY + 46);
            graphics.text(this.font, Component.literal("Eye size limit reached"), panelX, messageY, 0xFFFF6060);
        }
    }
