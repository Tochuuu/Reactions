...
public final class ReactionsEyelidOptionsScreen extends Screen {
    ...
    private void drawPreview(GuiGraphics graphics, ResourceLocation texture, ReactionsClientConfig config, boolean closed) {
        graphics.blit(texture, previewX, previewY, previewSize, previewSize, FACE_U, FACE_V, FACE_PIXELS, FACE_PIXELS, SKIN_SIZE, SKIN_SIZE);
        if (closed) {
    ...
    private void drawEyelidTile(GuiGraphics graphics, ResourceLocation texture, int x, int y, int width, int height, int tintAlpha, ReactionsClientConfig config) {
        graphics.blit(texture, x, y, width, height, clamp(config.eyelidColorX, 0, SKIN_SIZE - 1), clamp(config.eyelidColorY, 0, SKIN_SIZE - 1), 1, 1, SKIN_SIZE, SKIN_SIZE);
        if (tintAlpha > 0) {
