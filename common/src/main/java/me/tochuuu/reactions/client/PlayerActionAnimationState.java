...
public final class PlayerActionAnimationState {
    ...
    public static void capture(AbstractClientPlayer player, PlayerRenderState state, float partialTick) {
    ...
    private static FallState fallState(AbstractClientPlayer player, PlayerRenderState state) {
        int entityId = player.getId();
    ...
    private static float mountedYawDelta(AbstractClientPlayer player, PlayerRenderState state, float partialTick) {
        if (!state.isPassenger) {
            return 0.0F;
        }

        Entity vehicle = player.getVehicle();
        float referenceYaw = vehicle == null ? player.getYRot(partialTick) : vehicle.getYRot(partialTick);
