...
public final class PlayerEyeRenderLayer extends RenderLayer {
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int light, Entity entity, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        int eyeLook = animationsEnabled && !blinking ? blockFocusEye != 0 ? blockFocusEye : mountedBackEye != 0 ? mountedBackEye : mirroredIdleEye(player, ageInTicks) : 0;
        ...
        int overlay = eyelidTintEnabled ? LivingEntityRenderer.getOverlayCoords(player, 0.0F) : OverlayTexture.NO_OVERLAY;
        ...
        if (eyes.eyebrowsEnabled) {
            submitEyebrowAboveEye(poseStack, consumer, light, overlay, eyes.leftEyeX, eyes.leftEyeY, eyes.eyeWidth, eyes.leftEyeSkinLayer, eyebrowVerticalOffset(leftEye, eyes.eyeWidth, eyes.eyeHeight, hurtSclera));
    ...
    private static void submitEye(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, int skinY, int eyelidColorX, int eyelidColorY, int eyeWidth, int eyeHeight, ReactionsClientConfig.EyeSkinLayer eyeSkinLayer, ReactionsClientConfig.EyeSkinLayer eyelidColorSkinLayer, EyeExpression expression, int eyeLook, EyeSide side, boolean hurtSclera, boolean fallingSurprise, int eyelidColor, boolean texturedEyelids) {
        ...
        if (expression == EyeExpression.CLOSED) {
            submitEyelidTexture(poseStack, consumer, light, overlay, eyelidColorX, eyelidColorY, eyelidColorSkinLayer, eyeWidth, eyeHeight, dstX1, dstY1, dstX2, dstY2, eyelidColor, texturedEyelids);
            return;
        }

        if (expression == EyeExpression.OPEN && shouldExtendSclera(eyeWidth, eyeHeight, fallingSurprise)) {
            submitHurtScleraEye(poseStack, consumer, light, overlay, sourceSkinX, clampedSkinY, eyeWidth, eyeHeight, dstX1, dstY1, dstY2, side, true, shouldMirrorEyeColumns(eyeLook, side));
            return;
        }

        if (expression == EyeExpression.OPEN && canUseBlockEyeAnimation(eyeWidth, eyeHeight)) {
            submitBlockEye(poseStack, consumer, light, overlay, sourceSkinX, clampedSkinY, eyeWidth, eyeHeight, dstX1, dstY1, side, eyeLook, hurtSclera);
            return;
        }

        if (expression == EyeExpression.SQUINT) {
            submitSquintEye(poseStack, consumer, light, overlay, sourceSkinX, clampedSkinY, eyelidColorX, eyelidColorY, eyelidColorSkinLayer, eyeWidth, eyeHeight, dstX1, dstY1, dstY2, eyeLook, side, eyelidColor, texturedEyelids);
            return;
        }

        if (expression == EyeExpression.OPEN && shouldExtendSclera(eyeWidth, eyeHeight, hurtSclera)) {
            submitHurtScleraEye(poseStack, consumer, light, overlay, sourceSkinX, clampedSkinY, eyeWidth, eyeHeight, dstX1, dstY1, dstY2, side, false, false);
            return;
        }
        ...
        quad(consumer, poseStack.last(), dstX1, dstY1, dstX2, dstY2, u1, v1, u2, v2, light, overlay, NORMAL_COLOR);
    }

    private static void submitEyebrowAboveEye(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, int skinY, int eyebrowWidth, ReactionsClientConfig.EyeSkinLayer eyeSkinLayer, float dstYOffset) {
        ...
        eyebrowQuad(consumer, poseStack.last(), dstX1, dstY1, dstX2, dstY2, u1, v1, u2, v2, light, overlay, NORMAL_COLOR);
    }
    ...
    private static final class EMFCompatibility {
        ...
        private record FaceBounds(float minX, float minY, float minZ, float maxX, float maxY) {
            private static FaceBounds fromNorthFace(ModelPart.Cube cube) {
                ...
                return new FaceBounds(cube.minX, cube.minY, cube.minZ, cube.maxX, cube.maxY);
            }
    ...
    private static void pupilVertex(VertexConsumer consumer, PoseStack.Pose pose, float x, float y, float u, float v, int light, int overlay, int color) {
        consumer.vertex(pose.pose(), x / 16.0F, y / 16.0F, PUPIL_FACE_Z)
            .color(red(color), green(color), blue(color), alpha(color))
            .uv(u, v)
            .overlayCoords(overlay)
            .uv2(light)
    ...
    private static void eyebrowVertex(VertexConsumer consumer, PoseStack.Pose pose, float x, float y, float u, float v, int light, int overlay, int color) {
        consumer.vertex(pose.pose(), x / 16.0F, y / 16.0F, EYEBROW_FACE_Z)
            .color(red(color), green(color), blue(color), alpha(color))
            .uv(u, v)
            .overlayCoords(overlay)
            .uv2(light)
    ...
    private static void submitMouth(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, EyeSettings eyes) {
        ...
        submitMouthPixel(poseStack, consumer, light, overlay, eyes.leftMouthX, eyes.leftMouthY, eyes.mouthSkinLayer, dstX1, dstY1, dstX1 + 1.0F, dstY1 + 1.0F);
        submitMouthPixel(poseStack, consumer, light, overlay, eyes.rightMouthX, eyes.rightMouthY, eyes.mouthSkinLayer, dstX1 + 1.0F, dstY1, dstX1 + 2.0F, dstY1 + 1.0F);
    }
    ...
    private static void submitAdvancementMouth(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, EyeSettings eyes) {
        ...
        submitMouthPixel(poseStack, consumer, light, overlay, eyes.leftMouthX, eyes.leftMouthY, eyes.mouthSkinLayer, 1.25F, dstX1, dstY1, splitX, dstY1 + height + ADVANCEMENT_MOUTH_TOP_EXTENSION);
        submitMouthPixel(poseStack, consumer, light, overlay, eyes.rightMouthX, eyes.rightMouthY, eyes.mouthSkinLayer, 1.25F, splitX, dstY1, dstX1 + width, dstY1 + height + ADVANCEMENT_MOUTH_TOP_EXTENSION);
    }
    ...
    private static void submitUseMouth(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, EyeSettings eyes, MouthUseAnimation animation, float ageInTicks, int entityId) {
        ...
        if (animation == MouthUseAnimation.EATING) {
        ...
        float splitX = dstX1 + width * 0.5F;
        submitMouthPixel(poseStack, consumer, light, overlay, eyes.leftMouthX, eyes.leftMouthY, eyes.mouthSkinLayer, 1.0F, dstX1, dstY1, splitX, dstY1 + height);
        submitMouthPixel(poseStack, consumer, light, overlay, eyes.rightMouthX, eyes.rightMouthY, eyes.mouthSkinLayer, 1.0F, splitX, dstY1, dstX1 + width, dstY1 + height);
    }
    ...
    private static void submitMouthPixel(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, int skinY, ReactionsClientConfig.EyeSkinLayer skinLayer, float dstX1, float dstY1, float dstX2, float dstY2) {
        submitMouthPixel(poseStack, consumer, light, overlay, skinX, skinY, skinLayer, 1.0F, dstX1, dstY1, dstX2, dstY2);
    }

    private static void submitMouthPixel(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, int skinY, ReactionsClientConfig.EyeSkinLayer skinLayer, float sourceHeight, float dstX1, float dstY1, float dstX2, float dstY2) {
    ...
    private static void submitBlockEye(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, int skinY, int eyeWidth, int eyeHeight, float dstX1, float dstY1, EyeSide side, int eyeLook, boolean hurtSclera) {
        ...
        submitBlockEyeRow(poseStack, consumer, light, overlay, skinX, skinY, eyeWidth, eyeHeight, eyeHeight, dstX1, eyeDstY1, eyeDstY2, side, eyeLook);
    }

    private static void submitBlockEyeRow(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, float sourceY, int eyeWidth, int eyeHeight, float sourceHeight, float dstX1, float dstY1, float dstY2, EyeSide side, int eyeLook) {
        int scleraSourceColumn = externalScleraColumn(side, eyeWidth);
        for (int column = 0; column < eyeWidth; column++) {
            submitEyePiece(poseStack, consumer, light, overlay, skinX + scleraSourceColumn, sourceY, 1.0F, sourceHeight, dstX1 + column, dstY1, dstX1 + column + 1.0F, dstY2, NORMAL_COLOR);
        }
        ...
        if (eyeLook == DIRECT_BLOCK_FOCUS_UP_SIGNAL || eyeLook == DIRECT_BLOCK_FOCUS_DOWN_SIGNAL) {
            ...
            submitPupilPiece(poseStack, consumer, light, overlay, skinX + pupilSourceColumn, pupilSourceY, pupilWidth, visibleSourceHeight, dstX1 + pupilColumn, pupilDstY1, dstX1 + pupilColumn + pupilWidth, pupilDstY2, NORMAL_COLOR);
            return;
        }

        submitPupilPiece(poseStack, consumer, light, overlay, skinX + pupilSourceColumn, sourceY, pupilWidth, sourceHeight, dstX1 + pupilColumn, dstY1, dstX1 + pupilColumn + pupilWidth, dstY2, NORMAL_COLOR);
    }
    ...
    private static void submitEyelidTexture(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int eyelidX, int eyelidY, ReactionsClientConfig.EyeSkinLayer eyelidSkinLayer, int tileColumns, int tileRows, float dstX1, float dstY1, float dstX2, float dstY2, int color, boolean texturedEyelids) {
    ...
    private static void submitSquintEye(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, int skinY, int eyelidX, int eyelidY, ReactionsClientConfig.EyeSkinLayer eyelidSkinLayer, int eyeWidth, int eyeHeight, float dstX1, float dstY1, float dstY2, int eyeLook, EyeSide side, int eyelidColor, boolean texturedEyelids) {
        ...
        submitEyelidTexture(poseStack, consumer, light, overlay, eyelidX, eyelidY, eyelidSkinLayer, eyeWidth, eyelidSourceHeight, dstX1, dstY1, dstX2, splitY, eyelidColor, texturedEyelids);
        ...
        if (canUseBlockEyeAnimation(eyeWidth, eyeHeight)) {
            submitBlockEyeRow(poseStack, consumer, light, overlay, skinX, sourceY1, eyeWidth, eyeHeight, sourceVisibleHeight, dstX1, splitY, dstY2, side, eyeLook);
            return;
        }
    ...
    private static void submitHurtScleraEye(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int skinX, int skinY, int eyeWidth, int eyeHeight, float dstX1, float dstY1, float dstY2, EyeSide side, boolean surprise, boolean mirrored) {
        ...
        if (surprise && eyeWidth == 2 && eyeHeight == 2) {
            submitEyePiece(poseStack, consumer, light, overlay, externalSourceX, skinY, 1.0F, 1.0F, dstX1, extendedDstY1, dstX1 + eyeWidth, dstY1, NORMAL_COLOR);
            return;
        }
        ...
        quad(consumer, poseStack.last(), dstX1, extendedDstY1, dstX1 + eyeWidth, dstY1, externalU1, externalV1, externalU2, externalV2, light, overlay, NORMAL_COLOR);
        ...
        for (int column = 0; column < eyeWidth; column++) {
            ...
            quad(consumer, poseStack.last(), finalDstX1, overlayDstY1, finalDstX2, dstY2, u1, v1, u2, v2, light, overlay, NORMAL_COLOR);
        }
    }
    ...
    private static int mountedBackEye(AbstractClientPlayer player, float partialTick) {
        if (!player.isPassenger()) {
            return 0;
        }

        Entity vehicle = player.getVehicle();
        float referenceYaw = vehicle == null ? Mth.rotLerp(partialTick, player.yRotO, player.getYRot()) : Mth.rotLerp(partialTick, vehicle.yRotO, vehicle.getYRot());
        float headYaw = Mth.rotLerp(partialTick, player.yHeadRotO, player.yHeadRot);
        float yawDelta = Mth.wrapDegrees(headYaw - referenceYaw);
    ...
    private static int blockFocusEye(AbstractClientPlayer player, boolean isSelf) {
        int directFocus = isSelf ? BlockInteractionEyeFocus.localDirectFocusSignal() : ReactionsNetworking.remoteEyeFocus(player.getId());
