...
public final class PlayerEyeRenderLayer extends RenderLayer<PlayerRenderState, PlayerModel> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int light, PlayerRenderState state, float limbSwing, float limbSwingAmount) {
        ...
        if (!isSelf && remoteConfig == null && !useDefaultOfflineEyes && (playerOverride == null || !playerOverride.enabled)) {
            if (config.showMouth) {
                renderMouthOnly(poseStack, bufferSource, light, state, renderType(texture), EyeSettings.local(config));
            }
            return;
        }
    ...
    private void renderMouthOnly(PoseStack poseStack, MultiBufferSource bufferSource, int light, PlayerRenderState state, RenderType renderType, EyeSettings eyes) {
        poseStack.pushPose();
        getParentModel().head.translateAndRotate(poseStack);
        int overlay = OverlayTexture.pack(0.0F, state.hasRedOverlay);
        submitMouth(poseStack, bufferSource, renderType, light, overlay, eyes);
    ...
    private static void submitEye(PoseStack poseStack, MultiBufferSource bufferSource, RenderType renderType, int light, int overlay, int skinX, int skinY, int eyelidColorX, int eyelidColorY, int eyeWidth, int eyeHeight, EyeExpression expression, boolean mirrored) {
    ...
    private static void submitMouthCover(PoseStack poseStack, MultiBufferSource bufferSource, RenderType renderType, int light, int overlay, EyeSettings eyes, float dstX1, float dstY1, float width, float height) {
        ...
        mouthQuad(bufferSource.getBuffer(renderType), poseStack.last(), dstX1, dstY1, dstX1 + width, dstY1 + height, u1, v1, u2, v2, light, overlay, NORMAL_COLOR);
    }
