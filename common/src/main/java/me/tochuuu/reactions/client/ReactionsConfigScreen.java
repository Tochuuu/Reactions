...
public final class ReactionsConfigScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(this.font, this.title, this.width / 2 - this.font.width(this.title) / 2, 8, 0xFFFFFFFF);
        graphics.text(this.font, Component.literal("Skin UV"), faceX, faceY - 12, 0xFFE6E6E6);
        ...
        graphics.text(this.font, Component.literal("Active: " + mode.label), faceX, labelY, mode.color);
        graphics.text(this.font, Component.literal("Eyes " + config.eyeWidth + "x" + config.eyeHeight), faceX, labelY + 11, 0xFFBFC7D5);

        graphics.text(this.font, Component.literal("Animation"), panelX, panelY + 8, 0xFFE6E6E6);
        graphics.text(this.font, Component.literal("UV parts"), panelX, partHeaderY, 0xFFE6E6E6);
        graphics.text(this.font, Component.literal("Eye size"), panelX, sizeHeaderY, 0xFFE6E6E6);
        graphics.text(this.font, Component.literal("Width: " + config.eyeWidth), panelX, eyeWidthRowY + 6, 0xFFBFC7D5);
        graphics.text(this.font, Component.literal("Height: " + config.eyeHeight), panelX, eyeHeightRowY + 6, 0xFFBFC7D5);

        if (detailsY > 0 && detailsY < this.height - 36) {
            graphics.text(this.font, Component.literal("Left eye  " + uv(config.leftEyeX, config.leftEyeY)), panelX, detailsY, 0xFF43D17C);
            graphics.text(this.font, Component.literal("Right eye " + uv(config.rightEyeX, config.rightEyeY)), panelX, detailsY + 11, 0xFF4AA3FF);
            graphics.text(this.font, Component.literal("Mouth     " + uv(config.leftMouthX, config.leftMouthY) + " " + uv(config.rightMouthX, config.rightMouthY)), panelX, detailsY + 22, 0xFFFFD45A);
            graphics.text(this.font, Component.literal("Lid color " + uv(config.eyelidColorX, config.eyelidColorY)), panelX, detailsY + 33, 0xFFFFC94A);
        }

        if (sizeLimitMessageTicks > 0) {
            graphics.text(this.font, Component.literal("Eye size limit reached"), panelX, Math.min(this.height - 38, detailsY + 46), 0xFFFF6060);
        }
    }
